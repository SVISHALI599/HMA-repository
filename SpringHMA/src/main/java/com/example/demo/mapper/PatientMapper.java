...
public interface PatientMapper {
	@Select("SELECT pk_user_id as user_id,user_password, "
			+ "user_name, user_age, user_gender, user_mobile_number, user_email_id, user_address_line1, user_address_line2, user_address_line3, patient_disease FROM t_user JOIN t_patient ON t_user.pk_user_id=t_patient.fk_user_id")

	List<Patient> getAllPatient();

	@Select("select * from t_user t join t_patient p on t.pk_user_id=p.fk_user_id where (pk_user_id=#{pkPatientId} )")
	...
	@Insert("INSERT INTO t_patient (pk_patient_id,fk_user_id,patient_disease) VALUES (#{pkPatientId},#{userId},#{patientDisease})")
